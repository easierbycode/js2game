...
import { B, V } from "Engine/obj_types.js";
...
export function loadAnimGroup(name) {
    ...
    move_set.push({sprite:slot, width:slot[0].width, height:slot[0].height, count:framelist.length});
};

function drawBoundingBox(){
    if (char_side == 0){
        Draw.rect(World2Screen(char).x-move_set[move_state].width/2, 
                          World2Screen(char).y, 
...
export function drawCharacter() {
    ...
    if(char_side == 0){
        ...
        move_set[move_state].sprite[frame].draw(
            World2Screen(char).x-move_set[move_state].width/2-7.0, 
            World2Screen(char).y-3.5
        )
        move_set[move_state].sprite[frame].width = move_set[move_state].width*char_scale*1.1;
        move_set[move_state].sprite[frame].height = move_set[move_state].height*char_scale*1.05;
        ...
        move_set[move_state].sprite[frame].width = move_set[move_state].width*char_scale;
        move_set[move_state].sprite[frame].height = move_set[move_state].height*char_scale;
